...
export const Footer: React.FC<FooterProps> = ({ onNavigate }) => {
  ...
  const navGroups: { title: string; items: { label: string; page: PageId }[] }[] = [
    {
      title: 'Pages',
      items: [
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 py-16">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 lg:gap-8">
          ...
          <div className="md:col-span-3">
            ...
            <ul className="space-y-2.5">
              {capabilities.map(c => (
                <li key={c} className="text-[12.5px] text-slate-400">{c}</li>
              ))}
            </ul>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="space-y-2.5 text-[12.5px]">
              <a href={`mailto:${PERSONAL_INFO.email}`} className="block text-slate-400 hover:text-amber-400 transition-colors break-all">
                {PERSONAL_INFO.email}
              </a>
              <a href={`tel:${PERSONAL_INFO.phone.replace(/\s+/g, '')}`} className="block text-slate-400 hover:text-amber-400 transition-colors">
                {PERSONAL_INFO.phone}
              </a>
