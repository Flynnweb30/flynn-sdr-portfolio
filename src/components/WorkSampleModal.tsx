    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      >
        ...
        <div className="p-7 sm:p-10 border-b border-[rgba(91,168,221,0.10)] pr-16">
          <div className="text-[11px] font-mono text-brand-400/90 uppercase tracking-wider mb-5">
            {sample.category}
          </div>
          <h2 id="work-sample-title" className="text-[24px] sm:text-[30px] font-bold text-white leading-[1.15] tracking-tight">
            {sample.title}
          </h2>
          <p className="mt-4 text-[14px] text-ink-300 leading-[1.75]">{sample.summary}</p>
        </div>

        <div className="p-7 sm:p-10 space-y-8">
          <div>
            <h3 className="text-[11px] font-mono text-ink-300 uppercase tracking-wider mb-3">Overview</h3>
            <p className="text-[13.5px] text-ink-200 leading-[1.8]">{sample.details.overview}</p>
          </div>

          <div>
            <h3 className="text-[11px] font-mono text-ink-300 uppercase tracking-wider mb-4">Framework</h3>
            <div className="space-y-2.5">
              {sample.details.framework.map((step, i) => (
                <div key={i} className="p-4 bg-navy-800/60 border border-[rgba(91,168,221,0.10)] rounded-lg text-[13px] text-ink-100 leading-relaxed">
                  {step}
                </div>
              ))}
            </div>
          ...
          {sample.details.sampleText && (
            <div>
              <h3 className="text-[11px] font-mono text-ink-300 uppercase tracking-wider mb-3">Verbatim sample</h3>
              <div className="p-4 bg-brand-400/[0.05] border border-brand-400/20 rounded-lg text-[13px] text-ink-100 leading-relaxed font-mono">
                {sample.details.sampleText}
              </div>
            </div>
          )}

          <div className="p-5 bg-navy-800/60 border border-[rgba(91,168,221,0.10)] rounded-lg">
            <div className="flex items-center gap-2 mb-4">
              <Lightbulb className="w-3.5 h-3.5 text-brand-400" />
              <h3 className="text-[11px] font-mono text-ink-200 uppercase tracking-wider">Execution tips</h3>
            </div>
            <ul className="space-y-2.5">
              {sample.details.tips.map((tip, i) => (
                <li key={i} className="flex items-start gap-3 text-[13px] text-ink-200 leading-relaxed">
                  <span className="mt-2 w-1 h-1 rounded-full bg-brand-400/80 shrink-0" />
                  <span>{tip}</span>
                </li>
              ))}
            </ul>
        ...
        <div className="p-6 sm:p-7 border-t border-[rgba(91,168,221,0.10)] bg-navy-900/60 flex flex-col sm:flex-row items-center justify-between gap-4">
          <button
            onClick={handleCopy}
            className="w-full sm:w-auto px-4 py-2.5 text-[12.5px] font-medium text-ink-200 hover:text-white bg-navy-800 border border-[rgba(91,168,221,0.14)] rounded-md transition-colors inline-flex items-center justify-center gap-2"
          >
            {copied ? (
              <>
