...
export function useSEO(config: SEOConfig) {
  useEffect(() => {
    ...
    const setMeta = (selector: string, attr: string, value: string, create = false) => {
    ...
    // Canonical
    setMeta('link[rel="canonical"]', 'href', config.canonical.startsWith('http') ? config.canonical : `${SITE}${config.canonical}`, true);
    ...
    setMeta('meta[property="og:url"]', 'content', config.canonical.startsWith('http') ? config.canonical : `${SITE}${config.canonical}`, true);
  ...
  }, [config.title, config.description, config.canonical, config.ogImage, config.ogType, config.noindex, config.keywords]);
}

export const SEO_CONFIGS = {
  home: {
    title: 'Flynn James — B2B Appointment Setting & Cold Calling Specialist',
    description: 'Senior B2B SDR with 11+ years of outbound experience. $1.8M+ pipeline sourced and 30+ qualified meetings booked monthly for SaaS, agencies, and IT firms.',
    canonical: '/',
    keywords: 'B2B appointment setting, cold calling specialist, SDR, lead generation, outbound sales, B2B sales development',
  },
  about: {
    title: 'About Flynn James — 11+ Years in B2B Outbound Sales',
    description: 'Meet Flynn James, a Senior B2B SDR and Junior Sales Team Lead with 11+ years of cold calling, appointment setting, and SDR coaching experience.',
    canonical: '/about',
  ...
  services: {
    ...
    description: 'Six focused B2B outbound services: appointment setting, high-volume cold calling, lead generation, SDR coaching, LinkedIn outreach, and CRM pipeline management.',
    canonical: '/services',
    keywords: 'B2B appointment setting services, cold calling services, lead generation services, SDR services, outbound sales services',
  },
  experience: {
    title: 'Career Timeline — 11+ Years of B2B Sales Development',
    description: 'Flynn James career timeline: 11+ years of outbound sales across US, UK, ANZ, and Singapore markets. Consistent 120–150% quota attainment.',
  ...
  caseStudies: {
    ...
    description: 'Detailed case studies of outbound campaigns: $1.8M pipeline sourced for a UK agency, 22% demo conversion for enterprise SaaS, and Level 4 ramp in 3 weeks.',
    canonical: '/case-studies',
    keywords: 'B2B sales case studies, appointment setting results, cold calling case studies, outbound pipeline results',
  },
  samples: {
    title: 'Sales Playbooks & Cold Call Scripts — Flynn James',
    description: 'Free sales playbooks: cold call scripts, 7-touch multi-channel cadences, BANT qualification scorecards, and AE handoff templates used to source $1.8M+ pipeline.',
    canonical: '/samples',
  ...
  contact: {
    ...
    description: "Get in touch with Flynn James for B2B appointment setting, cold calling, or SDR coaching. Free 20-minute pipeline audit. Response within 24 hours.",
    canonical: '/contact',
