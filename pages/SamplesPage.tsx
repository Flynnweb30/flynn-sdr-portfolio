          ...
          {WORK_SAMPLES.map((sample, i) => {
            ...
            return (
              <motion.div
                ...
                className="group cursor-pointer bg-slate-900/30 hover:bg-slate-900/60 border border-slate-800/60 hover:border-slate-700/80 rounded-xl p-7 sm:p-9 transition-all"
              >
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  <div className="lg:col-span-8">
                    ...
                    <p className="text-[14px] text-slate-400 leading-[1.75]">
                      {sample.summary}
                    </p>

                    <div className="mt-6 flex flex-wrap gap-2">
                      {sample.tags.map(tag => (
                        <span key={tag} className="px-2.5 py-1 text-[11px] font-mono bg-slate-800/60 border border-slate-700/60 text-slate-400 rounded">
                          {tag}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="bg-[#0b0f19] border border-slate-800/60 rounded-lg p-4 h-full">
                      ...
                      <div className="mt-5 pt-4 border-t border-slate-800/60 flex items-center justify-between">
                        <button
                          ...
                          className="text-[11.5px] font-medium text-slate-400 hover:text-white transition-colors inline-flex items-center gap-1.5"
                        >
                          {isCopied ? (
                            <>
